...
class UCBalgorithm:
    def __init__(self, num_arms, burn_in=1, min_range=-float("inf"), max_range=float("inf"), epsilon=0, delta=0.1):
    ...
    def get_ucb_arm(self, confidence_radius, arm_info=None):
        if sum(self.counts) <= self.burn_in:
            ...
            ucb_bonuses = [confidence_radius * np.sqrt(np.log((self.global_time_step + 1.0) / self.delta) / (count + 1e-10)) for count in self.counts]
